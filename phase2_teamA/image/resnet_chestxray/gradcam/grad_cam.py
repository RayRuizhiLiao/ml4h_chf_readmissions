...
class _BaseWrapper(object):
    ...
    def _encode_one_hot(self, ids):
        one_hot = torch.zeros_like(self.probs).to(self.device)
        one_hot.scatter_(1, ids, 1.0)
        return one_hot

    def forward(self, image):
        self.image_shape = image.shape[2:]
        self.probs = self.model(image)[0] 
        # [0] was added because our model output has more than one channel
        # self.probs = F.softmax(self.logits, dim=1)
        # assuming our model has already added softmax 
        return self.probs.detach() # .sort(dim=1, descending=True)  # ordered results
    ...
    def backward(self, ids):
        ...
        self.probs.backward(gradient=one_hot, retain_graph=True)
